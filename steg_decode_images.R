...
data_list <- lapply(files, function(x) {
  data <- read.table(x, header = FALSE, stringsAsFactors=FALSE)  ## Read in data from Dario Beraldi's Short Reference Global Aligner
  plus_data <- data[data$V4 == "+", ]                 ## Contains all reads from plus strand
  minus_data <- data[data$V4 == "-", ]                ## Contains all reads from minus strand
  
  ## Set alignment quality threshold
  clean_plus_data <- plus_data[(plus_data$V3 >= quantile(plus_data$V3, quality_cut)) & (plus_data$V5 == seq_length), ]      
  clean_minus_data <- minus_data[(minus_data$V3 >= quantile(minus_data$V3, quality_cut)) & (minus_data$V5 == seq_length), ] 
  ...
  averaged_data <- (processed_plus + processed_minus) / 2      ## Average output of plus and minus data
})
...
for (j in seq(length(message_portion))) {
  ...
  for (i in 1:length(message_portion[[j]][1,])){
    ...
    else if ( (message_portion[[j]]["C", i] > t1) & (message_portion[[j]]["C", i] < t2) 
      & (message_portion[[j]]["T", i] > t1) & (message_portion[[j]]["T", i] < t2) ) {result[[j]][i] <- 1}
    else if ( (message_portion[[j]]["G", i] > t1) & (message_portion[[j]]["G", i] < t2) 
      & (message_portion[[j]]["A", i] > t1) & (message_portion[[j]]["A", i] < t2) ) {result[[j]][i] <- 0} 
    else {result[[j]][i] <- 3} ## Catch unassigned positions for troubleshooting purposes
  }
}
