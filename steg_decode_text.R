...
files <- list.files(pattern = "^.*\\.txt$")                     ## Working directory containing only SRGA output files 
...
data_list <- lapply(files, function(x) {
  ...
  clean_data <- data[(data$V3 >= quantile(data$V3, quality_cut)) & (data$V5 == seq_length), ]      ## Set alignment quality threshold
...
## Save the mC_list as an R object to be imported when run in mode == "BS"
