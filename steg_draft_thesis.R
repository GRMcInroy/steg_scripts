...
quality_cut <- 0.2                              ## Define the quality cut off
...
process <- function(x, y) {
      ...
      dg_perc <- dg                                             ## Create dummy data.frame ready for accepting base percentages at positions
      sapply(1:length(dg[1,]), FUN=function(i){ dg_perc[,i] <<- 100*dg[,i]/sum(dg[,i])})      ## Convert the matrix of base counts into % base at position
}
...
for (j in seq(length(data_list))) {
  ...
  for (i in 1:length(data_list[[j]][1, ])){                              
    ...
    else if ( (data_list[[j]]["C", i] > t1) & (data_list[[j]]["C", i] < t2) & (data_list[[j]]["T", i] > t1) & (data_list[[j]]["T", i] < t2) ) {result[[j]][i] <- "Y"}
    else if ( (data_list[[j]]["G", i] > t1) & (data_list[[j]]["G", i] < t2) & (data_list[[j]]["A", i] > t1) & (data_list[[j]]["A", i] < t2) ) {result[[j]][i] <- "R"}  
    else {result[[j]][i] <- "N"}
  }
}
...
for (i in seq(length(data_list))) {
    ...
    barplot(as.matrix(data_list[[i]]), col = c("blue2", "red", "chartreuse3","gold"), xlab = "Sequence Position", ylab = "% base", axes = TRUE, xlim = c(4, ncol(data_list[[i]]) + 18))  ## Plot the dg_perc matrix as a stacked barplot
    legend(58, 145, horiz = TRUE, fill = c("blue2","red","green3","yellow2"), legend = c("A", "T", "C","G"), bty = "n", xpd=TRUE)     ## Add a legend to the barplot
